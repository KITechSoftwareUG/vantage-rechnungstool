...
-- Drop existing restrictive policies and replace with authenticated-user policies
...
DROP POLICY IF EXISTS "Service role can manage all logs" ON public.document_ingestion_log;
...
CREATE POLICY "Service role can manage all logs" ON public.document_ingestion_log FOR ALL USING (true) WITH CHECK (true);
